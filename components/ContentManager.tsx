...
const ContentManager: React.FC<ContentManagerProps> = ({
  ...
  const handleSaveBlog = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      if (error) alert('Update Failed: ' + error.message)
        ...
        setBlogs(
          blogs.map((b) => (b.id === editingId ? { ...b, ...blogData } : b)),
        )
        cancelEdit()
      }
    } else {
      const { data, error } = await supabase
        .from('blogs')
        .insert([{ ...blogData, date: new Date().toLocaleDateString() }])
        .select()
  ...
  const handleSaveTestimonial = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      if (error) alert('Update Failed: ' + error.message)
        ...
        setTestimonials(
          testimonials.map((t) =>
            t.id === editingId ? { ...t, ...testimonialData } : t,
          ),
        )
        cancelEdit()
      }
  ...
  const inputClass =
    'w-full bg-white border border-zinc-200 rounded-xl p-3 text-sm text-zinc-900 placeholder:text-zinc-400 focus:ring-2 focus:ring-primary-500 outline-none transition-all shadow-inner'
        ...
        <div className='flex flex-col md:flex-row justify-between items-center gap-6 mb-12'>
          ...
          <div className='flex bg-white/10 p-1 rounded-2xl backdrop-blur-md border border-white/5'>
            <button
              ...
              className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'blogs' ? 'bg-primary-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}
            >
              Blogs
            </button>
            <button
              ...
              className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'testimonials' ? 'bg-primary-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}
            >
              Testimonials
            </button>
          </div>
            ...
            <div className='bg-white p-8 rounded-[2.5rem] shadow-2xl border border-zinc-100 sticky top-32'>
              ...
              {activeTab === 'blogs' ? (
                <form onSubmit={handleSaveBlog} className='space-y-4'>
                  {/* Image Upload */}
                  <div
                    className='relative aspect-video rounded-2xl overflow-hidden bg-zinc-100 border border-zinc-200 group cursor-pointer'
                    onClick={() => blogInputRef.current?.click()}
                  >
                    <NextImage
                      src={newBlog.image}
                      alt={newBlog.title || 'Blog image'}
                      fill
                    ...
                    <div className='grid grid-cols-2 gap-2'>
                      {['Technical', 'Innovation', 'Strategy'].map((cat) => (
                        <button
                          ...
                          type='button'
                          onClick={() =>
                            setNewBlog({ ...newBlog, category: cat })
                          }
                  ...
                  <button
                    type='submit'
                    disabled={isSaving}
                    className='w-full py-4 rounded-xl font-black text-[10px] uppercase tracking-widest bg-primary-600 text-white shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center gap-2'
                  >
                    {isSaving ? (
                  ...
                  {editingId && (
                    <button
                      type='button'
                      onClick={cancelEdit}
                      className='w-full text-[10px] font-black uppercase tracking-widest text-zinc-400'
                    >
                      Cancel
                    </button>
                  )}
                ...
                <form onSubmit={handleSaveTestimonial} className='space-y-4'>
                  <div className='flex flex-col items-center gap-4 mb-4'>
                    <div
                      className='w-20 h-20 rounded-full overflow-hidden border-2 border-primary-500 relative group cursor-pointer'
                      onClick={() => testimonialInputRef.current?.click()}
                    >
                      <NextImage
                        src={newTestimonial.image}
                        alt={newTestimonial.name || 'Testimonial image'}
                        fill
                        className='object-cover'
                      />
                    ...
                    <div className='flex gap-1'>
                      {[1, 2, 3, 4, 5].map((s) => (
                        <Star
                          key={s}
                          size={20}
                          className={`cursor-pointer ${s <= newTestimonial.rating ? 'fill-amber-400 text-amber-400' : 'text-zinc-200'}`}
                          onClick={() =>
                            setNewTestimonial({ ...newTestimonial, rating: s })
                          }
                        />
                      ))}
                    </div>
                  ...
                  <button
                    type='submit'
                    disabled={isSaving}
                    className='w-full py-4 rounded-xl font-black text-[10px] uppercase tracking-widest bg-emerald-600 text-white shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center gap-2'
                  >
                    {isSaving ? (
                      <Loader2 size={14} className='animate-spin' />
                    ) : editingId ? (
          ...
          <div className='lg:col-span-2 space-y-4'>
            {isLoading ? (
              ...
              blogs.map((blog) => (
                <div
                  key={blog.id}
                  className='bg-white p-4 rounded-3xl border border-zinc-100 flex items-center justify-between shadow-sm hover:shadow-md transition-shadow'
                >
              ...
              testimonials.map((t) => (
                ...
                >
                  ...
                  <div className='flex gap-2'>
                    <button
                      onClick={() => handleEditClick(t, 'testimonial')}
                      className='p-2 text-amber-600 bg-amber-50 rounded-lg hover:bg-amber-100 transition-colors'
                    >
                      <Edit2 size={16} />
                    </button>
