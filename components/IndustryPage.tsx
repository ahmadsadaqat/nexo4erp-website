...
const IndustryPage: React.FC<IndustryPageProps> = ({
  ...
  const openModal = (e: React.MouseEvent, type: 'demo' | 'consult') => {
    ...
    const subject =
      type === 'demo'
        ? `Industry Demo: ${industry.name}`
        : `Consultation: ${industry.name}`
  ...
  return (
    <div className='bg-transparent min-h-screen pb-20'>
      ...
      <section className='relative min-h-[70vh] flex items-center pt-32 pb-20 lg:pt-40 lg:pb-32 overflow-hidden'>
        ...
        <button
          onClick={onBack}
          className='absolute top-24 left-4 md:left-8 z-20 flex items-center gap-2 text-gray-500 hover:text-primary-600 dark:text-gray-400 dark:hover:text-primary-400 transition-colors bg-white/50 dark:bg-black/30 backdrop-blur-md px-3 py-1.5 rounded-full border border-gray-200 dark:border-zinc-800'
        >
        ...
        <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
          <ScrollReveal>
            <h2 className='text-3xl font-bold text-gray-900 dark:text-white mb-12 text-center'>
              Critical Challenges in {industry.name}
            </h2>
          </ScrollReveal>
          <div className='grid grid-cols-1 md:grid-cols-3 gap-8'>
            {industry.painPoints.map((point, i) => (
              <ScrollReveal key={i} delay={i * 100}>
                <div className='p-8 rounded-2xl bg-gray-50/80 dark:bg-zinc-900/80 backdrop-blur-md border border-gray-100 dark:border-zinc-800 hover:shadow-lg transition-shadow'>
                  <div className='w-10 h-10 bg-red-100 dark:bg-red-900/20 rounded-lg flex items-center justify-center text-red-600 dark:text-red-400 mb-4 font-bold border border-red-200 dark:border-red-900/40 shadow-sm'>
                    !
                  </div>
                  ...
                  <p className='text-gray-500 dark:text-gray-400 text-sm leading-relaxed'>
                    <span className='text-primary-600 dark:text-primary-400 font-bold italic mx-1'>
                      The Result:
                    </span>{' '}
                    {point.result}
                  </p>
          ...
          <ScrollReveal>
            <div className='text-center mb-16'>
              <h2 className='text-3xl font-bold text-gray-900 dark:text-white mb-4'>
                Strategic NEXO Editions for {industry.name}
              </h2>
              <p className='text-gray-500 dark:text-gray-400 text-lg'>
                Modular frameworks designed for institutional scale.
              </p>
            </div>
          </ScrollReveal>
          <div className='grid grid-cols-1 md:grid-cols-3 gap-8'>
            {industry.packages.map((pkg, i) => (
              <ScrollReveal key={i} delay={i * 100}>
                <div className='bg-white/70 dark:bg-zinc-900/70 backdrop-blur-md rounded-2xl p-8 border border-gray-100 dark:border-zinc-800 h-full flex flex-col shadow-sm hover:shadow-xl transition-all duration-300'>
                  <h3 className='text-xl font-bold text-gray-900 dark:text-white mb-2'>
                    {pkg.name}
                  </h3>
                  <div
                    className='text-3xl font-extrabold text-primary-600 dark:text-primary-400 mb-6'
                  >
                    {pkg.price}
                  </div>
                  ...
                  <button
                    onClick={(e) => {
                      ...
                      window.dispatchEvent(
                        new CustomEvent('open-contact-modal', {
                          detail: {
                            subject: `Package Inquiry: ${pkg.name}`,
                          },
                        }),
                      )
                    }}
      ...
      <section className='py-24'>
        <div className='max-w-5xl mx-auto px-4'>
          <ScrollReveal>
            <div className='bg-gradient-to-br from-primary-700/90 via-primary-800/90 to-primary-950/90 backdrop-blur-md rounded-3xl p-12 text-center text-white shadow-2xl relative overflow-hidden group'>
              <div className='absolute inset-0 bg-white/5 opacity-0 group-hover:opacity-100 transition-opacity duration-700'></div>

              <div className='relative z-10'>
                <h2 className='text-3xl md:text-5xl font-bold mb-6'>
                  Elevate Your {industry.name} Operations
                </h2>
