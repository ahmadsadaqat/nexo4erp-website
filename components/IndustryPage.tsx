...
const IndustryPage: React.FC<IndustryPageProps> = ({
  ...
  const openModal = (e: React.MouseEvent, type: 'demo' | 'consult') => {
    ...
    const subject =
      type === 'demo'
        ? isArabic
          ? `طلب عرض تجريبي للصناعة: ${industry.name}`
          : `Industry Demo: ${industry.name}`
        : isArabic
          ? `استشارة متخصص: ${industry.name}`
          ...
          <ScrollReveal>
            <div className='max-w-4xl rtl:text-right'>
              <div className='inline-flex items-center px-4 py-2 rounded-full bg-primary-600/10 dark:bg-primary-500/10 text-primary-700 dark:text-primary-300 font-bold text-xs uppercase tracking-widest mb-8 border border-primary-500/20 backdrop-blur-md'>
                {isArabic ? 'حلول الصناعة:' : 'Industry Solution:'}{' '}
                {industry.name}
              </div>
        ...
        <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
          <ScrollReveal>
            <h2 className='text-3xl font-bold text-gray-900 dark:text-white mb-12 text-center'>
              {isArabic
                ? `التحديات الحرجة في ${industry.name}`
          ...
          <ScrollReveal>
            <div className='text-center mb-16'>
              <h2 className='text-3xl font-bold text-gray-900 dark:text-white mb-4'>
                {isArabic
                  ? `إصدارات نيكسو الإستراتيجية لـ ${industry.name}`
                  : `Strategic NEXO Editions for ${industry.name}`}
              </h2>
              <p className='text-gray-500 dark:text-gray-400 text-lg'>
                {isArabic
                  ? 'أطر عمل معيارية مصممة لنطاق مؤسسي.'
                    ...
                    onClick={(e) => {
                      ...
                      window.dispatchEvent(
                        new CustomEvent('open-contact-modal', {
                          detail: {
                            subject: isArabic
                              ? `استفسار عن حزمة: ${pkg.name}`
                              : `Package Inquiry: ${pkg.name}`,
                          },
                        }),
                      )
                    }}
            ...
            <div className='bg-gradient-to-br from-primary-700/90 via-primary-800/90 to-primary-950/90 backdrop-blur-md rounded-3xl p-12 text-center text-white shadow-2xl relative overflow-hidden group rtl:text-right'>
              ...
              <div className='relative z-10'>
                <h2 className='text-3xl md:text-5xl font-bold mb-6'>
                  {isArabic
                    ? `ارتقِ بعمليات ${industry.name} الخاصة بك`
                ...
                <div className='flex flex-col sm:flex-row gap-4 justify-center'>
                  ...
                  >
                    {isArabic
                      ? 'ابدأ إصدار العرض تجريبي المجاني'
